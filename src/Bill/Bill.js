...
const REACT_APP_BASE_API_URL=process.env.REACT_APP_BASE_API_URL;
  ...
  const saveBill = async () => {
    ...
    await Promise.all(
      ...
      axios.post(`${REACT_APP_BASE_API_URL}/sales/add`, {
  venderId: item.vid,              // vendor number (or change to ObjectId if you use vendor model)
  productId: item._id,             // use MongoDB product id
  quantity: quantities[item.pid] || 1,
  totalPrice: item.oprice * (quantities[item.pid] || 1),

}).then((res)=>{
  alert("Sales "+res.data)
}).catch((err)=>{
  alert(err);
});
