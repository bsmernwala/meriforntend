...
const REACT_APP_BASE_API_URL=process.env.REACT_APP_BASE_API_URL;
  ...
  const fetchVenders = () => {
    axios.get(REACT_APP_BASE_API_URL+"/vender/getvendercount")
  ...
  const toggleStatus = (vid, currentStatus) => {
    ...
    axios.put(`${REACT_APP_BASE_API_URL}/vender/vendermanage/${vid}/${newStatus}`)
      .then(res => {
        alert(res.data);
        fetchVenders();
      })
  ...
  const handleEditSave = async () => {
    ...
    axios.put(`${REACT_APP_BASE_API_URL}/vender/update/${selectedVender.VUserId}`, formData)
      .then(res => {
      ...
      {showModal && (
        <div style={{
          ...
          <div style={{ backgroundColor: "#fff", padding: "20px", borderRadius: "8px", minWidth: "300px" }}>
            <h3>Edit Vendor</h3>
            <input type="text" name="VenderName" value={editForm.VenderName} onChange={handleEditChange} placeholder="Vendor Name" /><br/>
            <input type="text" name="VAddress" value={editForm.VAddress} onChange={handleEditChange} placeholder="Address" /><br/>
            <input type="text" name="VContact" value={editForm.VContact} onChange={handleEditChange} placeholder="Contact" /><br/>
            <input type="email" name="VEmail" value={editForm.VEmail} onChange={handleEditChange} placeholder="Email" /><br/>
